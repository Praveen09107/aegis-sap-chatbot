...
**OPEN-12 — `vision_task.py` never received the retrofit `DEC-040` announced; the already-shipped chat screenshot feature is currently broken under the live default `INFERENCE_MODE=external`.** Confirmed directly (DEC-048): the file makes a raw, unbranched call to `OLLAMA_VISION_URL`, which doesn't exist under the default config. `AMENDMENT_INFERENCE_ARCHITECTURE.md`'s `FILE 4b` still contains `DEC-038`'s superseded "NO CHANGE REQUIRED" text rather than the real retrofit `DEC-040` said had been written. Needs: `FILE 4b` rewritten with an actual retrofit (branch on `INFERENCE_MODE`, preserving the real local-mode `/api/chat` call shape, per `DEC-040`'s note that this differs from `ollama_vision.py`'s `/api/generate` shape) and then applied to the real file. **Higher priority than it looks — this affects a feature already in production use, not a not-yet-built one.**

**OPEN-13 — Cerebras/Groq API keys are not just "missing benchmark data" (OPEN-05) — the actual call path returns a hard `401 Unauthorized`, confirmed live.** Found during Session 23 (DEC-048) testing Quick Entry's Prerequisite 1. Every external-inference call — main reasoning, judge, and vision, across the entire application, not just vision — is currently non-functional in this environment until real values replace the placeholder `CEREBRAS_API_KEY`/`GROQ_API_KEY` in `secrets-share/.env`. `OPEN-05` remains open for the separate, narrower question of live throughput benchmarks once real keys exist.
