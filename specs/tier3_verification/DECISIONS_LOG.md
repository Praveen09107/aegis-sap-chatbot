...
### DEC-057 — Full Re-Verification Pass, Sessions 25-29: Quick Entry Content Was Never Actually Retrievable (`chunk_id` missing from Qdrant payload); Bulk-Import Parser Rebuilt Against the Real Template; IMPL_28 Section 5 (Screenshot Surfacing) Built for the First Time

**Status:** CONFIRMED

**Decision:** A dedicated, from-scratch re-verification pass across every Session 25-29 deliverable, run against genuinely live containers (real HTTP calls, real service interruptions, real concurrent requests, real payload inspection) rather than code review — per explicit instruction not to just "confirm the function exists." Found and fixed five real defects, in ascending order of severity, plus built one genuinely missing capability the pass's own final test required.

**1. `form_import_parser.py` (Session 29's bulk-import parser) was built against a fictional document structure.** Live `.docx` upload testing surfaced four compounding bugs: `detect_content_type()` keyed on fields (`CAUSE_DESCRIPTION`/`ISSUE_DESCRIPTION`) that don't exist in the real template, made undetectable by a deliberately malformed test section; `_extract_between()` matched a label as a substring of a longer label, then — even after a first `\b`-boundary fix — still matched the same word appearing inside another field's own prose value, root-caused to case-insensitive, non-anchored matching. Fixed by switching detection to `WHEN_THIS_OCCURS` (confirmed real and error_guide-specific) and anchoring `_extract_between`/`_extract_steps` to line-start, case-sensitive — matching this codebase's own established `FIELD_LABEL_PATTERN` convention. `_extract_cause_blocks()`/`parse_error_guide()` were then found to assume a shared `CAUSE_DESCRIPTION`/`HOW_TO_IDENTIFY`/`RESOLUTION_STEPS` trio that has no basis in `AEGIS_DOCUMENT_TEMPLATES.md` (the real, frozen source of truth) — rewritten against the real per-cause-prefixed structure (`CAUSE_N_HOW_TO_IDENTIFY`/`CAUSE_N_RESOLUTION_STEPS`). `parse_procedure()`'s `verification` field also had a truncated end-label (`COMMON_ERRORS` instead of the real `COMMON_ERRORS_IN_THIS_PROCEDURE`), silently bleeding the next field's content into it. 14 new unit tests pin all four fixes, including the malformed-input and substring/word-match regressions specifically.

**2. A malformed JSON request body produced an unhandled `500` on any endpoint, not `IMPL_25`'s documented `400`.** Systemic gap, not endpoint-specific — fixed with one global `app.exception_handler(json.JSONDecodeError)` in `main.py`.

**3. CRITICAL — no Quick Entry content has been retrievable by any employee query since Session 26.** `_stage5_rrf_fusion` (`retrieval_engine.py`) silently drops any chunk whose payload lacks a `chunk_id` key (`if not chunk_id: continue`) — confirmed empirically by querying a live chunk's actual Qdrant payload, where the key was simply absent. `process_form_entry.py` and `retry_partial_indexing.py` both fixed by adding `"chunk_id": point_id` (reusing the same id OpenSearch's own `chunk_id` field already used, rather than inventing a second scheme). A related, smaller payload-key mismatch fixed alongside it: Quick Entry payloads write `verified_date`, not the `last_verified_date` key document-pipeline chunks use, silently leaving freshness/attribution dates empty for every Quick Entry source — fixed with a fallback in `_stage5_rrf_fusion`.

**4. IMPL_28 Section 5 (screenshot surfacing in the CRAG/attribution pipeline) had never been built in any prior session** — confirmed by grep (zero results for `screenshot_ids`/`has_screenshots` anywhere in `retrieval_engine.py`), not assumed. This blocked the pass's own required final end-to-end test (an employee query must display the attributed screenshot). Flagged directly rather than silently building or silently skipping; explicitly authorized to build now. Added `source_type`/`form_entry_id`/`has_screenshots`/`screenshot_ids` to `RetrievedChunk` (`models/retrieval.py`), populated from payload in `_stage5_rrf_fusion`, and a new `_fetch_screenshot_metadata()` in `validation_engine.py` that batch-fetches proxy-ready screenshot metadata (deduped by URL, capped at 5) for `build_attribution_panel()`, which now also returns `form_entry_id` and `screenshots`.

**5. Two further real bugs surfaced specifically by running the pass's own final end-to-end test** (a real Quick Entry with an attached screenshot, created and published through the actual API, then queried through the real retrieval pipeline) — neither would have been caught by unit tests alone, since both are integration-level indexing/enforcement gaps:
   - OpenSearch's chunk document (`process_form_entry.py` Stage A11) never included `screenshot_ids` at all — only `has_screenshots`. Inconsistent with `retry_partial_indexing.py`'s equivalent OpenSearch write, which already included it correctly. Since `_stage5_rrf_fusion` hydrates a chunk's payload from whichever source list it first appears in (dense Qdrant results checked before BM25), this only manifests for chunks ranked via BM25 alone — but it's a genuine indexing inconsistency between the two write paths regardless. Fixed by adding the missing field.
   - **`is_current` was written correctly by the retirement step (Stage A3, on every version bump) but never enforced by any retrieval stage.** Neither `_stage1_dense`, `_stage2_identity` (Qdrant), nor `search_bm25` (OpenSearch) filtered on it — confirmed by grep returning zero results for `is_current` anywhere in `retrieval_engine.py` or `opensearch_client.py`'s query construction. Practical effect: every retired version of an edited Quick Entry chunk remained permanently retrievable and could outrank the current version, since RRF scores stale duplicates independently. Confirmed this field is exclusively a Quick Entry concept (grep across `backend/app/` found it in no document-ingestion file, only Quick Entry task/handler files) — so this is in-scope for Sessions 25-29, not a wider pre-existing document-pipeline defect. Fixed with an **exclude** filter (`is_current != False`), not a blanket `is_current == True` require-filter: `qdrant_client.search_content()` gained an `exclude_conditions` parameter building a `must_not` clause, and `search_bm25()` gained an equivalent `must_not: {"term": {"is_current": False}}` clause. An exclude-only filter was the deliberate choice — a require-filter would have matched Qdrant/OpenSearch's actual semantics of "field absent never matches `MatchValue`," which would have silently zeroed out any hypothetical document-pipeline content that has never set the field at all, since both content types share the same Qdrant collections.

**Real, live evidence for the final end-to-end test (after all five fixes above):** a real Quick Entry (`SD-ERR-E2E-FINAL`) created as a draft via `POST /api/admin/knowledge-entries` (`201`), a screenshot attached (necessarily via the same disclosed direct-MinIO/DB workaround used throughout this whole feature's verification, since `OPEN-13` blocks real vision classification), published via `PUT .../{id}` (`200`, `status: processing` → confirmed `active`), with `processing_log` showing both `qdrant_insertion` and `opensearch_indexing` as `status: success` for both chunks. Directly invoking `RetrievalEngine.retrieve()` with a real BGE-embedded query returned exactly the entry's 2 current-version chunks (zero stale duplicates, zero cross-entry leakage after the `is_current` fix and an orphaned-test-data cleanup sweep), with `cause_1` correctly carrying `has_screenshots=True` and the right `screenshot_ids`, `error_overview` correctly carrying none. `build_attribution_panel()` returned the correct `form_entry_id` and a `screenshots` array with the correct proxy URL, caption, and section. The LLM-generated-answer half of the originally-requested test (an employee's natural-language question, answered and citing this source) remains blocked by `OPEN-13` — not silently skipped, the retrieval/attribution mechanics that were actually broken are what this pass targeted and fixed.

**A disclosed testing artifact, not a product defect:** an earlier draft of this same end-to-end test was accidentally published without a screenshot attached, then cleaned up via a direct Postgres `DELETE` rather than a real API call — which left 6 orphaned points in `meridian_errors` (Qdrant) and their `sap_documents` (OpenSearch) counterparts, since Quick Entry has no code path that reconciles hard-deleted rows against the vector/search stores (only archival is a real, supported operation). These were the first entries the `is_current` fix's own re-verification surfaced as unexpected duplicate/cross-entry results before their true cause (my own test cleanup, not a code defect) was confirmed and all 6 were removed from both stores.

**Also confirmed, matching the pass's explicit checklist, no defects found:** `chunker` respects logical structure and `config_values`'s never-split rule for Quick Entry; `retry_partial_indexing` genuinely detects and completes a deliberately-interrupted partial-index state (service stopped mid-run, not simulated); 9 ARQ tasks registered; zero `llava`/duplicate `VISION_SERVICE_URL`-style constants anywhere in the vision pipeline (confirmed `OLLAMA_VISION_URL` has exactly one definition, in `config.py`); `store_diagnostic_object()` never called from Quick Entry code; all three lifecycle-eligibility branches independently correct; proxy route is the only screenshot access path; `qe_rate:{user_id}` namespace has no collision; bulk import fails gracefully per-entry, not all-or-nothing; pipeline health reflects genuine live state; gap write-back updates the correct originating event; rate limiting produces a real `429`; both concurrent-edit races (re-confirmed from `DEC-056`) still correctly produce one `200`/one `409`.

**Affects:** `backend/app/services/form_import_parser.py`, `backend/app/main.py`, `backend/app/tasks/process_form_entry.py`, `backend/app/tasks/retry_partial_indexing.py`, `backend/app/models/retrieval.py`, `backend/app/services/retrieval_engine.py`, `backend/app/services/validation_engine.py`, `backend/app/infrastructure/qdrant_client.py`, `backend/app/infrastructure/opensearch_client.py`, `tests/unit/test_form_import_parser.py` (new), `tests/unit/test_validation_engine.py`.
...
**OPEN-05 — RESOLVED by DEC-060.** ~~Live benchmark numbers for the three selected inference models remain uncaptured.~~ Real, unmocked latency and format-compliance data captured for all 13 tier/role combinations across all 5 providers — see `specs/tier3_verification/INFERENCE_BENCHMARK_RESULTS_2026-07-19.md`.

**OPEN-06 — Still open (re-attempted and extended by DEC-064, still not closeable on real evidence — see DEC-064 for the full attempt and a real, separate RPD/RPM mislabel bug it found and fixed along the way).** SambaNova's actual free-tier numeric rate limits remain unverified. See DEC-022. `DEC-060`'s benchmark deliberately checked this directly rather than assuming: a real 200 response from both SambaNova tiers (`main`, `judge`) carries zero rate-limit headers of any kind — confirming the code's own `quota_kind: "sliding_window"` design assumption ("no headers for this provider") is correct, but this doesn't reveal what the real ceiling actually is. The existing "20 RPM" figure in `circuit_breaker.py`'s override comment remains an unverified assumption, not evidence — `DEC-060`'s benchmark deliberately paced SambaNova conservatively (~6 RPM) rather than trusting that number as fact.
...
**OPEN-08 — RESOLVED by DEC-065.** ~~`AEGIS_DATA_CONTRACTS.md` documents a `"correction"` WebSocket message type that `IMPL_17` never actually implements sending.~~ Confirmed the real judge (`run_judge_evaluation()`) only ever produces 3 numeric scores, never free-text correction content, and `validate_with_regeneration()`'s real behavior is a full-answer replacement already delivered via `validation_result.answer_text` (`DEC-059`) — no real gap existed to fill. Removed from the data contract, `IMPL_19`, and the real (already dead) frontend type union rather than built as unused code.
...
**OPEN-16 — Standing, not yet actioned: rotate all 5 provider API keys, then do the real Oracle Cloud deployment (`REMAINING_BACKEND_ITEMS_GUIDE.md` Items 4/5).** Both require real human action this environment cannot perform on its own: Item 4 needs an interactive login to 5 separate external dashboards (Groq, Cerebras, SambaNova, Cloudflare, Gemini) to generate fresh keys — this session (`DEC-066`) built and live-verified the zero-restart rotation mechanism those new keys would flow through, but did not and could not generate the keys themselves. Item 5 needs a real Oracle Cloud account and real infrastructure provisioning. Do Item 4 once fresh keys exist (rotate via Vault, confirm no restart needed — mechanism ready); do Item 5 last, per the guide's own ordering, once everything else is genuinely complete.

**OPEN-13 — RESOLVED by DEC-059.** ~~Cerebras/Groq API keys are not just "missing benchmark data" (OPEN-05) — the actual call path returns a hard `401 Unauthorized`.~~ Real (development-use, pre-rotation) keys for all 5 providers supplied 2026-07-19; every external inference call path — main reasoning, judge, vision — confirmed live and working end-to-end for the first time in this project's history. See DEC-059 for the full real-key verification pass and everything it found. `OPEN-05` remains open for the separate, narrower question of live throughput benchmarks.

---

### DEC-058 — N-Tier Inference Orchestration Built: Real Vision-Constant Bug Found Live, Per-Process Circuit State Confirmed as a Known Limitation, Not a New Bug

**Status:** CONFIRMED

**Decision:** Implemented `INFERENCE_ORCHESTRATION_ARCHITECTURE_PLAN.md` in full (Phases 0 through 6) — a declarative N-tier chain registry (`app/config_inference_chains.py`), wire-format adapters for Cloudflare and Gemini (`app/infrastructure/providers_cloudflare.py`, `providers_gemini.py`), per-provider circuit-breaker overrides, an atomic Redis-backed quota tracker (Lua `EVAL` for the sliding-window providers, closing the TOCTOU race a naive check-then-act port of `check_qe_rate_limit` would have had), the `walk_chain()` routing engine, a bounded pre-first-byte fallback for the streaming main-reasoning path, a new ARQ cron health-monitor task, migration 011, and a new admin endpoint. Full design already recorded in the plan document; this entry records what live implementation and testing found beyond the plan itself.

**A real, live bug found and fixed while wiring the registry (Phase 0), independent of everything else in this work:** `config.py`'s `GROQ_MODEL_VISION` still held `"meta-llama/llama-4-scout-17b-16e-instruct"` — confirmed, via a real `GET /v1/models` call during the earlier model-selection research this same day, that Llama 4 Scout no longer exists on Groq's live catalog. Every real vision call through the *existing, already-shipped* 2-provider cascade in `ollama_vision.py`/`vision_task.py` was silently 404ing on Groq before ever reaching this session's new code. Fixed to `"qwen/qwen3.6-27b"`, the actual finalized vision primary confirmed live earlier — Llama 4 Scout itself is still genuinely used, just moved to Cloudflare's chain slot, unaffected by this fix.

**The two duplicated 2-provider vision cascades (`ollama_vision.py`'s `_run_vision_prompt`, `vision_task.py`'s `_run_vision_extraction`) were retired in favor of `walk_chain()`,** exactly as planned — confirmed a third call site existed beyond the two originally known (`knowledge_screenshots_handler.py` and `enrich_entry_screenshots.py` both call *into* `ollama_vision.py`, not directly into the retired cascade code), so the real blast radius was 3 call sites through 2 file changes, not 2. The swallow-vs-raise error-handling asymmetry between the two owning files was deliberately left untouched, per the plan's Design Principle 6 — only cascade internals changed.

**Circuit-breaker state confirmed live to be per-process, in-memory only — a pre-existing architectural fact, not a defect introduced or fixed by this work.** While verifying the new admin endpoint, tripping a circuit via a separate `docker exec python3` process did not appear in the running FastAPI server's own state — because it isn't the same process, and `uvicorn` itself runs 2 worker processes (`UVICORN_WORKERS=2`) that each hold their own independent `circuit_registry` singleton. This means a circuit can legitimately show OPEN in one worker and CLOSED in the other simultaneously, and any single request to the new admin endpoint reflects whichever worker happened to handle it. This was already true of every circuit this codebase has ever tracked (Qdrant, OpenSearch, the original Cerebras/Groq pair) — the new inference-orchestration circuits inherit the same characteristic, nothing new. Making circuit state cross-process (e.g., Redis-backed) was never in this plan's scope and is not proposed here — named explicitly so it isn't mistaken for something this session broke.

**Verified live, not assumed:** full container rebuild and restart, `/health` green, zero import errors across all new modules, ARQ worker log directly confirms `check_inference_provider_health` registered in both the function list and as `cron:check_inference_provider_health`. The new `GET /api/admin/inference-health` endpoint hit live with a real IT-admin JWT — returns the correct 4-main/4-judge/5-vision tier structure, and correctly distinguishes the two pre-registered override circuits (`sambanova_main`/`sambanova_judge`, immediately visible as `"closed"`) from every lazily-created circuit (`null` until first real use) — direct, live confirmation that Phase 2's pre-registration change works as designed. 321 unit tests passing (up from 261 before this session's Quick Entry verification pass and the subsequent model research), all against mocked responses built from real, live-captured shapes — no automated test makes a live external call, per the plan's own testing constraint.

**Deliberately not exercised in this pass, disclosed rather than silently skipped:** the plan's own "Definition of Done" whole-system acceptance test (deliberately invalidate one tier's key, confirm the request still succeeds via fallback, confirm the admin endpoint and metric both reflect it) requires real, working API keys for Groq/Cerebras/SambaNova/Cloudflare/Gemini. The keys available in this environment are the same ones pasted in plaintext chat earlier this session and explicitly flagged for rotation — used earlier for read-only research verification, but deliberately not exercised through the new production routing/quota code without the developer's explicit confirmation that rotation has happened. This is the same `OPEN-13`-shaped blocker this project has carried since Session 23, now extended to the four new providers.

**Affects:** `backend/app/config.py` (vision-constant fix, new cascade-budget/ceiling constants), `backend/app/config_inference_chains.py` (new), `backend/app/infrastructure/providers_cloudflare.py` (new), `backend/app/infrastructure/providers_gemini.py` (new), `backend/app/infrastructure/inference_providers.py` (header-exposing return-type change), `backend/app/infrastructure/circuit_breaker.py` (per-provider overrides), `backend/app/infrastructure/redis_client.py` (quota tracker methods), `backend/app/services/model_gateway.py` (`walk_chain()`, `generate_streaming()`/`call_judge()` external-mode rewire, local/Ollama paths unchanged), `backend/app/clients/ollama_vision.py`, `backend/app/tasks/vision_task.py`, `backend/app/tasks/check_inference_provider_health.py` (new), `backend/app/handlers/inference_health_handler.py` (new), `backend/app/main.py` (router registration), `backend/app/observability.py` (4 new metrics), `backend/app/workers/arq_worker.py` (new cron entry), `database/migrations/011_inference_provider_health.sql` (new), `infrastructure/grafana/dashboards/aegis-main.json` (3 new panels), 6 new test files plus `tests/unit/test_ollama_vision.py` updates.

---

### DEC-059 — First Real End-to-End Inference Pass: Critical Streaming Bug Found and Fixed, N-Tier Failover Proven Live, OPEN-13/OPEN-15 Resolved

**Status:** CONFIRMED

**Decision:** With the developer's explicit, one-time authorization to use the current (unrotated) real API keys for development/testing purposes only — rotation before actual deployment is a standing obligation, not optional, see the reminder at the end of this entry — ran the first genuinely real, unmocked end-to-end pass through the entire backend: real retrieval, real CRAG/judge assessment, real multi-provider generation through `walk_chain()`, real validation, real vision extraction, and real delivery to a WebSocket client. `OPEN-13` (placeholder keys returning hard 401s) had blocked every prior attempt at this since Session 23 — this is the first time any of these code paths has run against live providers.

**CRITICAL, pre-existing, previously-undiscoverable bug found: the Redis Pub/Sub token relay never existed.** `reasoning_service.generate_and_stream()` has always published every token to the `stream:{session_id}` Redis Pub/Sub channel via `redis_session.publish_token`/`publish_stream_complete`, but a full-codebase grep confirmed nothing anywhere ever subscribed to that channel. **No real employee has ever seen a streamed answer arrive in their browser since this feature was originally built** — undiscoverable until now because `OPEN-13` blocked every real generation call before this session, so the pipeline never ran far enough to expose the gap. Fixed by adding `_relay_pubsub_stream_to_websocket()` to `backend/app/handlers/chat_handler.py`, run as a concurrent `asyncio.create_task()` alongside the generation call (started before, not after, generation begins publishing, and bounded by a 5s `asyncio.wait_for` safety net in case `stream_complete` itself never lands). Verified live: real tokens streamed in real time, forming a complete, accurate answer. 4 new regression tests added (`backend/tests/unit/test_chat_handler_streaming.py`) covering the happy path, mid-stream client disconnect, malformed-message resilience, and unsubscribe-on-error — all mocked, no real server, specifically to catch a regression back to the pre-fix silent-no-op state.

**Related bug found and fixed in the same pass: the final `validation_result` WebSocket message never included the answer text at all.** Even setting aside the Pub/Sub gap, an employee would see a validation score/badge/attribution panel but never the actual answer. Fixed by adding `"answer_text": validation_result.answer_text` to that message. This also closes a real consistency gap: `validation_engine.validate_with_regeneration()` can produce a genuinely different final answer via a direct (non-Pub/Sub-publishing) call to `model_gateway.generate_streaming()` when the first attempt scores below amber — without this field, a client would never see that regenerated answer reflected anywhere.

**CRAG's two live outcomes both verified working correctly, for the first time:** a real, answerable question against a real (Quick-Entry-seeded) document correctly produced `SUFFICIENT` and a real generated answer; a deliberately unanswerable question correctly produced `INSUFFICIENT`, the ticket-escalation ARQ task queued, and the client-facing error message delivered with `ticket_id: None` — confirmed correct by design (`_queue_ticket_task` is fire-and-forget via ARQ, not synchronous, so there is no ID to return in the immediate response).

**N-tier failover (DEC-058's core deliverable) proven live, not just unit-tested:** deliberately broke the primary vision provider and confirmed via container logs that `walk_chain()` correctly detected the failure and cascaded to the next tier, a completely different wire-format adapter, producing output of identical quality to the primary. Real, live proof the resilience design works under an actual provider failure, not a mocked one.

**Vision extraction proven live and flawless** on a synthetic SAP screenshot, confirmed via `classify_sap`/`extract_sap_content` returning fully correct structured fields.

**Second real, latent gap found and fixed: `secrets-share/.env`'s `KEYCLOAK_CLIENT_SECRET` still held the placeholder `REPLACE_client_secret`, first flagged live in `OPEN-15`.** Confirmed `backend/app/config.py` never actually reads this variable — the FastAPI backend only verifies JWT signatures against Keycloak's JWKS, it never performs a client-secret grant itself — so this was never a live backend bug, only a latent one that would have broken the not-yet-built frontend's own ROPC login exchange the moment it existed. Corrected `.env` to the value Keycloak is actually configured with (`aegis_chat_client_secret_dev`, confirmed live via a real token request), rather than rotating Keycloak, since nothing else in the stack references the old placeholder value. `OPEN-15` resolved.

**A genuine operational finding, not a code defect:** `docker compose restart` does not pick up `.env` changes — it reuses a container's already-resolved environment. Only `docker compose up -d` (after a config change) re-resolves `env_file` contents on container recreation. Discovered mid-failover-test when a deliberately-broken key kept succeeding; corrected by using `up -d` and confirming via `docker exec ... env` that the change had actually taken effect before re-testing.

**Full endpoint sweep against real backing services, all confirmed working:** `pipeline-health`, `knowledge-entries` list, `suggest-doc-id`, `validate-reference`, and `check-duplicate` (the last exercising a real BGE embedding call and a real Qdrant search, not mocked) all hit with a real IT-admin JWT and returned correct results. `inference-health` re-confirmed the same per-worker circuit-state characteristic already disclosed in `DEC-058` (not a new bug — `UVICORN_WORKERS=2` means each worker holds its own in-memory `circuit_registry`).

**Verified live, not assumed:** full existing suite plus the 4 new tests — 325 passed, 0 failed (up from 321). `docker compose config --quiet` valid. All 17 containers healthy. All real test artifacts created during this pass (one Quick Entry document, 2 attached screenshots, their Postgres rows, Qdrant points, OpenSearch documents, and MinIO objects) confirmed fully removed — verified via direct row counts (all 0) and a real MinIO object listing (0 remaining) after cleanup, not assumed from the delete calls' return values alone.

**Standing reminder, not yet actioned — the developer's own explicit instruction when authorizing real-key use for this pass:** the 5 provider keys now live in `secrets-share/.env` (Groq, Cerebras, SambaNova, Cloudflare, Gemini) are the same ones used for this session's testing and must be rotated to fresh values before actual production deployment. Not done as part of this entry — flagged here so it isn't forgotten, per explicit request.

**Affects:** `backend/app/handlers/chat_handler.py` (Pub/Sub relay fix, `answer_text` field, stale module docstring corrected), `backend/tests/unit/test_chat_handler_streaming.py` (new, 4 tests), `secrets-share/.env` (`KEYCLOAK_CLIENT_SECRET` corrected — gitignored, not part of any commit).

---

### DEC-060 — Vault Decision Scoped (Not Deleted), `.env.example` Audit Completed, Real Rate-Limit-Honest Inference Benchmark Run

**Status:** CONFIRMED

**Decision:** Closes out the remaining "before frontend" open items from `DEC-059`'s next-steps list: `OPEN-14` (Vault's fate), `OPEN-10` (`.env.example` staleness), `OPEN-05` (real benchmark numbers). The initial plan for this pass recommended deleting `vault_client.py` outright and pacing the benchmark against existing code-comment rate-limit figures — both were directly challenged and corrected before any code was touched; the revised approach below is what actually shipped.

**On Vault (`OPEN-14`): challenged and reconsidered before acting.** The first draft's "just delete it" recommendation would have left `aegis-vault` (the container), `scripts/setup_vault.py`, and its engine provisioning sitting dead-but-undeleted — not a real resolution, just moving the "why does this exist" question elsewhere. Asked directly: the chosen direction is to **repurpose** Vault's already-working AppRole auth flow for provider API-key secrets management (KV v2 engine) rather than remove it or leave it dead — this targets a real, freshly-surfaced pain point (`DEC-059`'s own key-rotation reminder: rotating 5 keys today means hand-editing `.env` and recreating containers) with infrastructure that's already deployed and already has a working auth path. **Not built in this pass** — it's a genuine multi-file feature (enable KV v2 in `setup_vault.py`, rewrite `vault_client.py`'s Postgres-shaped API into a generic `get_secret(path)`, decide fetch/cache/refresh strategy, decide whether it fully replaces `.env` for these 5 keys, and specifically design rotation so updating a secret in Vault doesn't still require a full container recreation) — scoped here as a named follow-up rather than rushed into a cleanup-and-benchmark session. Confirmed while scoping this: migration 004's `aegis_app_role` group is unrelated and untouched either way (a real, load-bearing static Postgres permission group PgBouncer's `aegis_pooled_role` inherits from — never something Vault managed). No files changed for this item.

**On `.env.example` (`OPEN-10`): the broader audit that entry itself asked for, actually done.** Direct diff against the real, working `secrets-share/.env` found three real drifts, not the one originally flagged: (1) `KEYCLOAK_CLIENT_ID=aegis-backend` → `aegis-chat`; (2) Postgres routing entirely bypassed PgBouncer (`POSTGRES_HOST=aegis-postgres-primary`, port `5432`, user `aegis_user` — all pre-`DEC-051` values that would fail against the real pooled-auth architecture) → corrected to `aegis-pgbouncer`/`6432`/`aegis_pooled_role`; (3) `DATABASE_URL` confirmed dead (`grep -rn "DATABASE_URL" backend/app` — zero references anywhere) and internally self-inconsistent even on its own terms (hardcoded `aegis_user`, contradicting the real `aegis_pooled_role` two lines above it in the same file) — deleted rather than fixed, matching the "delete confirmed-dead code" call already made for `vault_client.py`'s candidacy. One more confirmed-dead placeholder found in the same pass: `APP_SECRET_KEY` (`grep -rn "APP_SECRET_KEY" backend/app` — zero references; this project's auth is entirely Keycloak/JWT-based) — also deleted. Checked and deliberately left alone: `OPENSEARCH_USER`/`PASSWORD` (unused by design, OpenSearch's security plugin is disabled), `NEXTAUTH_SECRET` (correctly still a placeholder — no frontend exists yet to use it). All 4 fixes applied to both `.env.example` (committed) and `secrets-share/.env` (local-only, gitignored) where applicable.

**On the benchmark (`OPEN-05`): challenged and reconsidered on rate-limit assumptions before running anything.** The first draft would have paced Cerebras/SambaNova calls against numbers taken from existing code comments without checking whether those comments were ever independently verified — directly challenged, since `OPEN-06` already flags SambaNova's limit as unconfirmed and Cerebras's figure had never been checked against a live header either. Revised: `scripts/aegis_inference_benchmark.py` fully rewritten to iterate the real `INFERENCE_CHAINS` registry and call `model_gateway._dispatch_tier_nonstreaming()` directly — the same dispatch function `walk_chain()` uses in production, not a separate reimplementation (the pre-`DEC-058` version of this script only covered 2 of 5 providers and called a Groq vision model `DEC-058` had already found was 404ing). A Step 0 probe fired one real, minimal-token call per provider first and captured the complete raw response header set — not just the subset the app's own quota tracker currently parses — before any pacing decision was made.

**Real rate-limit evidence obtained, not assumed:** Cerebras's real header shows `x-ratelimit-limit-requests-minute: 5` — directly, freshly reconfirms `DEC-019`'s "5 RPM" figure with live evidence rather than trusted provider docs. SambaNova returned zero rate-limit headers on a real 200 (both `main` and `judge` tiers) — confirms the code's own `quota_kind: "sliding_window"` design assumption empirically, but reveals nothing about the actual ceiling; `OPEN-06` stays open on that basis, and the benchmark paced SambaNova conservatively (~6 RPM) rather than trusting the existing "20 RPM" comment. Gemini's "5 RPM" is separately confirmed via a real `quotaValue: "5"` captured in a 429 error body during `DEC-058`'s own build (`providers_gemini.py::parse_quota_value`) — cited rather than re-induced, to avoid deliberately spending quota on a already-confirmed number. Full detail and the per-provider evidence table: `specs/tier3_verification/INFERENCE_BENCHMARK_RESULTS_2026-07-19.md`.

**Real latency and format-compliance data captured for all 13 tier/role combinations, 3 real repetitions each (39 timed calls, plus the 13-call probe and a 3-call follow-up — ~55 real calls total this session).** Highlights: every tier reachable and functional; Cerebras is the fastest real path in all three roles (0.71s/0.28s-tier-1-only/0.81s avg); two real findings surfaced, both disclosed rather than silently fixed or silently ignored:
- **Judge fallback tier 2 (`openai/gpt-oss-20b`) returns empty content at the real `CRAG_MAX_TOKENS=64` budget**, reproducibly (re-confirmed with a dedicated follow-up: 64→empty, 150→`SUFFICIENT`, 300→`SUFFICIENT`) — a reasoning-style model spending its entire token budget on hidden reasoning before the visible verdict. Traced through `retrieval_engine.py::_stage6_crag` and confirmed this degrades safely (empty content falls through the ambiguous-response branch to the existing "default to SUFFICIENT" behavior, not a crash) — but it's a real, disclosed reliability gap: if this tier is ever actually reached, it silently contributes no real judgment rather than a genuine sufficiency check. **RCA'd and fixed in `DEC-061`** — root-caused precisely (not just disclosed) and given a real per-tier fix rather than left as an accepted-but-degrading caveat.
- **This benchmark's own vision format-checker initially under-reported Groq vision's real compliance** — the model prefixed 2 of 3 responses with a `<think>...</think>` block before the JSON, and the script's original checker required the whole response to parse as JSON. The real production parser (`ollama_vision.py::_parse_extraction_response`) already handles this correctly via `find("{")`/`rfind("}")` substring extraction — confirmed by `DEC-059`'s own live vision test, which used that real parser and reported flawless extraction. Fixed the benchmark script's checker to match production's approach for future runs; not re-run against the fix in this pass, since production-path correctness was already independently confirmed. Groq vision also hit a genuine live `429` on its 3rd rapid call — real evidence its vision endpoint has a tighter effective ceiling than its main-reasoning endpoint despite sharing the same header shape.

**Verified live:** all 55 real calls this pass were genuine HTTP requests to genuine provider endpoints — no mocks. Full existing test suite + `docker compose config --quiet` + a full `aegis-fastapi`/`aegis-arq` recreation (`up -d`, not `restart`) after the `.env`/`.env.example` edits, confirming `/health` stayed green with the trimmed variable set.

**Affects:** `.env.example` (4 fixes), `secrets-share/.env` (`DATABASE_URL`/`APP_SECRET_KEY` removed — gitignored, not part of any commit), `scripts/aegis_inference_benchmark.py` (full rewrite), `specs/tier3_verification/INFERENCE_BENCHMARK_RESULTS_2026-07-19.md` (new). No changes to `vault_client.py`, `docker-compose.yml`, or `scripts/setup_vault.py` — see `OPEN-14`'s updated entry for the scoped-not-built follow-up.

---

### DEC-061 — Judge Tier 2 Empty-Content Root-Caused and Fixed: Per-Tier `min_max_tokens` Floor, Not a Global Budget Change

**Status:** CONFIRMED

**Decision:** `DEC-060`'s benchmark disclosed but deliberately didn't fix the judge fallback tier 2 (`openai/gpt-oss-20b`) empty-content finding, reasoning that "safe degradation" (falls through to CRAG's existing default-SUFFICIENT path) was enough for that pass. Challenged directly and correctly: a judge tier that always returns empty and therefore always defaults to SUFFICIENT contributes zero real judgment whenever it's actually reached — worse than having no fallback at all, since it looks like a second opinion happened when it didn't. "Doesn't crash" and "genuinely functional" are different bars, and this workstream doesn't count as closed until the real one is met.

**Root cause, confirmed with the raw API response, not inferred from behavior alone.** Groq's response for this model carries a `message.reasoning` field separate from `message.content`, and `usage.completion_tokens_details.reasoning_tokens` is counted against the same `max_completion_tokens` budget as the visible answer. A graduated real test (`max_tokens` = 16, 32, 48, 64, 96, 128) showed `finish_reason: "length"` (truncated) with empty `content` at every value through 96 — `reasoning_tokens` consuming 14 through 94 of the budget at each step — flipping to `finish_reason: "stop"` with `content='SUFFICIENT'` only at 128 (`reasoning_tokens=112`). This specific CRAG prompt needs roughly 112-124 completion tokens, almost entirely hidden reasoning, before this model reaches its visible verdict — `CRAG_MAX_TOKENS=64` was never enough, for any prompt of comparable shape, not just this one test case.

**Checked for a parameter-level fix before touching the budget — confirmed there isn't one.** Groq accepts a `reasoning_effort` parameter for this model. `'low'` was tested at `max_tokens=64`: no change, still 62 reasoning tokens, still empty. `'none'` is rejected outright specifically for this model (`"reasoning_effort must be one of low, medium, or high"`) despite a separate, more generic validation-error message elsewhere implying `'none'` was an allowed value — Groq's own error responses are inconsistent about this, but the model-specific rejection governs. There is no way to suppress this model's minimum reasoning overhead below what already exceeds `CRAG_MAX_TOKENS`.

**Fix: a per-tier `min_max_tokens` floor, added to the chain registry, not a change to the shared `CRAG_MAX_TOKENS` constant.** `config_inference_chains.py`'s judge tier 2 entry now carries `"min_max_tokens": 128`. `walk_chain()` (`model_gateway.py`) computes `max(caller_max_tokens, tier.get("min_max_tokens", caller_max_tokens))` immediately before dispatching each tier — every tier without the key is completely unaffected, `CRAG_MAX_TOKENS` stays 64 for tier 1/3/4 and for every other role's chain, and the override can only ever raise a tier's effective budget, never lower one that's already larger (verified by a dedicated test, below). This was chosen over swapping the model at this slot: `gpt-oss-20b` is Groq's own current recommended replacement for the deprecated `llama-3.1-8b-instant`, this is a fallback-only tier (only reached once tier 1 has already failed, so the ~2x token-cost increase never touches the hot path), and 128 is a directly-measured value from this exact investigation, not an estimate.

**Verified live, twice, at two different levels:** first against a `docker cp`-patched copy of the running container (proving the override computation itself is correct — 3/3 real reps returned `SUFFICIENT`, not empty) — then, since `aegis-fastapi`/`aegis-arq` have no source bind-mount, a real `docker compose build` + `up -d` recreation, followed by a fresh confirmation read of `INFERENCE_CHAINS["judge"][1]["min_max_tokens"]` from inside the *rebuilt* image, to make sure the fix is genuinely in the deployed artifact and not just a live-patched test copy that a future rebuild would silently discard. 3 new regression tests added (`tests/unit/test_model_gateway_walk_chain.py::TestWalkChainMinMaxTokensOverride`): the declaring tier receives the bumped value, every other tier keeps the caller's original budget unchanged, and a caller budget already larger than the floor is never reduced. Full suite: 328 passed (up from 325). `docker compose config --quiet` valid, `/health` green post-recreation.

**This closes the judge-tier-2 workstream (`OPEN-05`) without a caveat** — not "closed, but this one tier silently degrades," genuinely closed.

**Affects:** `backend/app/config_inference_chains.py` (`min_max_tokens` field + judge tier 2 entry), `backend/app/services/model_gateway.py` (`walk_chain()` override computation), `tests/unit/test_model_gateway_walk_chain.py` (3 new tests), `specs/tier3_verification/INFERENCE_BENCHMARK_RESULTS_2026-07-19.md` (judge section and summary rewritten with the full RCA, call-count table added — 67 real calls this session, fully itemized).
...
### DEC-064 — OPEN-06 (SambaNova Rate Limit): Real RPD/RPM Mislabel Found and Fixed; the Numeric Ceiling Itself Stays Genuinely Open

**Status:** CONFIRMED (partial) — `REMAINING_BACKEND_ITEMS_GUIDE.md` Item 1.

**What was actually tried, live, this session:** two of the three methods the item's own session prompt names. (1) SambaNova's developer dashboard requires an interactive login this environment has no credentials or browser access for — not attempted, disclosed rather than guessed at. (2) Fired 8 rapid real, minimal-token calls directly at SambaNova's `/chat/completions` in a tight loop, using the real key already in `secrets-share/.env`. All 8 returned `200`, and the captured raw headers carried zero rate-limit fields of any kind (`date`, `content-type`, `inference-id`, `x-request-id`, `strict-transport-security` — nothing else) — re-confirming `DEC-060`'s own finding, extended: no burst-level (per-second/per-minute) throttling is visible even at 8 rapid calls.

**A real, separate finding surfaced while investigating, worth fixing regardless of the numeric-ceiling question:** `backend/app/config.py`'s `SAMBANOVA_RPD_CEILING = 20` (comment: "confirmed via SambaNova's own rate-limits doc") is a genuine, already-wired, already-in-production ceiling — `model_gateway.py::_check_tier_quota()` calls `redis_session.reserve_sliding_window_quota(..., window_seconds=86400, max_requests=SAMBANOVA_RPD_CEILING)`, i.e. **a requests-per-DAY limit**, confirmed live in the real dispatch path. But `circuit_breaker.py`'s per-service override comment for `sambanova_main`/`sambanova_judge` said **"20 RPM/model"** — the same number, wrong unit, describing an entirely different (and already correctly-implemented-elsewhere) mechanism. This is a real, in-code contradiction that predates this session, not something introduced by it — fixed by correcting the comment to describe what the override is actually for (circuit-tripping on real call failures, independent of and unrelated to the sliding-window RPD gate) rather than restating a mislabeled number.

**Did not attempt method 3 (deliberately triggering a real 429 to exhaust the day's quota) beyond the 8-call burst test** — SambaNova is fallback-only in both chains (4th of 4 in `main`, 4th of 4 in `judge`, per the item's own stated low priority), and burning through an entire day's real 20-request ceiling purely to probe a number with no bearing on current functionality was judged disproportionate, especially with no independent (dashboard) way to confirm the resulting number wasn't itself an artifact of running out mid-burst rather than the true steady-state limit.

**OPEN-06 stays open, explicitly, per the item's own instruction not to close it on an estimate** — the confirmed-live facts are: no burst-level headers or throttling at 8 rapid calls, and a distinct, already-correctly-implemented 20/day ceiling whose "SambaNova's own rate-limits doc" provenance itself was never independently re-verified this session (inherited from before `DEC-060`, which only ever scrutinized the RPM figure, not this one). Nothing here justifies closing OPEN-06 as resolved.

**Affects:** `backend/app/infrastructure/circuit_breaker.py` (override comment corrected).

---

### DEC-065 — OPEN-08 Resolved: the `"correction"` WebSocket Message Was Never Buildable, Removed Rather Than Built

**Status:** RESOLVED — `REMAINING_BACKEND_ITEMS_GUIDE.md` Item 2.

**Decision, made before writing any code per the item's own instruction:** removed the `"correction"` message type from `AEGIS_DATA_CONTRACTS.md`, `IMPL_19_EMPLOYEE_FRONTEND.md`'s `WebSocketMessageType` union, and the real, already-shipped `frontend/src/types/index.ts`'s `WSMessageType` union (confirmed dead there too — no `=== "correction"` handling anywhere in the real frontend, via grep) — rather than building a sender for it.

**Why, confirmed by reading the real code, not inferred:** the data contract's own documented shape (`{"type": "correction", "message": "One correction to the above:", "correction_text": "..."}`) describes a narrow, targeted fact-fix appended to an otherwise-kept answer — categorically different from what `validate_with_regeneration()` actually does. Confirmed in `validation_engine.py`: (1) `run_judge_evaluation()`'s real Qwen prompt requests exactly 3 numeric scores (`faithfulness`, `step_completeness`, `relevance`) — no free-text correction field exists anywhere in the judge's real input or output shape, so `correction_text` could never be populated with real content without inventing a new judge prompt/response schema (out of scope — real architecture invention, not a gap-fill, per `CLAUDE.md` Rule 4). (2) Regeneration (triggered when `validation_score` scores `"none"`) re-runs the full reasoning model and, if better, returns a **wholly new** `regen_answer` that replaces `answer_text` outright — never a delta note over the original. This full-replacement path is already correctly delivered to the client via `validation_result`'s `answer_text` field, fixed in `DEC-059`. There is no real gap for `"correction"` to fill.

**Affects:** `specs/tier1_foundation/AEGIS_DATA_CONTRACTS.md` (section removed, replaced with an explanatory note), `specs/tier2_implementation/IMPL_19_EMPLOYEE_FRONTEND.md` (union member removed with the same note), `frontend/src/types/index.ts` (dead union member removed, confirmed via `tsc --noEmit` clean afterward).

---

### DEC-066 — Item 3 Built: Vault KV v2 Provider-Key Rotation, Live-Verified With Zero Container Restarts

**Status:** CONFIRMED — `REMAINING_BACKEND_ITEMS_GUIDE.md` Item 3, resolving `OPEN-14`'s scoped follow-up in full.

**A real, useful discovery before writing anything:** Vault's dev-mode container (`VAULT_DEV_ROOT_TOKEN_ID`, confirmed via `docker-compose.yml`) auto-mounts a KV v2 engine at `secret/` on every fresh start (confirmed live via `GET /sys/mounts` → `"secret/": {"type": "kv", "options": {"version": "2"}}`), and `scripts/setup_vault.py`'s existing `aegis-fastapi-policy` already grants `read`/`list` on `secret/*` — provisioning "from scratch" was mostly already half-done by dev-mode defaults; the real work was seeding it and building the read/refresh path around it.

**A second, load-bearing discovery: Vault dev mode is fully in-memory regardless of the mounted volume — every AppRole role_id/secret_id is wiped and regenerated on every container restart.** Confirmed live: the `VAULT_ROLE_ID`/`VAULT_SECRET_ID` already in `secrets-share/.env` failed a real AppRole login (`400`) despite the container being up 31 hours with no visible restart — meaning they were stale from a prior instance. Re-ran `scripts/setup_vault.py`, captured the freshly-generated pair, confirmed it logs in and reads real KV v2 data, and corrected `secrets-share/.env` in place (gitignored, local-only). **Operationally important for any future session**: `scripts/setup_vault.py` must be re-run — and `.env`'s Vault credentials re-synced — after any `aegis-vault` container restart/recreation, or every Vault-dependent feature (including this one) silently falls back to `.env` values (safe, but silently loses the "sourced from Vault" benefit) until re-provisioned.

**Built, in order:**
1. `scripts/setup_vault.py` — new `setup_kv_engine()`: idempotently enables KV v2 at `secret/` (harmless no-op against dev-mode's auto-mount, correct against a real non-dev Vault), seeds the 5 provider keys (`GROQ_API_KEY`, `CEREBRAS_API_KEY`, `SAMBANOVA_API_KEY`, `CLOUDFLARE_API_TOKEN`, `GEMINI_API_KEY`) from this script's own environment (`load_dotenv()` added, matching `config.py`'s convention) into `secret/aegis/inference-provider-keys`. Also fixed the script's already-exists detector, which didn't recognize Vault's real `"path is already in use"` 400 message (only checked for "already exists"/"already enabled") — a pre-existing gap this session's first real run surfaced.
2. `backend/app/infrastructure/vault_client.py` — `get_postgres_credentials()` removed (confirmed dead: zero callers anywhere in the codebase, per `DEC-051`), replaced with generic `get_secret(path)` / `put_secret(path, data)` KV v2 methods, reusing the same already-working AppRole login/token-caching machinery unchanged. Deliberately no silent `.env` fallback inside the client itself — that policy decision belongs to the caller (see next point), since not every future secret this client might read will have a sensible fallback.
3. `backend/app/config_inference_chains.py` — the actual rotation design. `INFERENCE_CHAINS` is built once at import time from `config.py`'s flat `os.getenv()` constants, same as always — a frozen dict entry can't itself support zero-restart rotation. Added `refresh_provider_keys()` (fetches all 5 keys from Vault, falls back per-key to the original `.env`-sourced value if Vault is unreachable or a key is missing, then mutates `INFERENCE_CHAINS`'s existing tier dicts' `"api_key"` fields **in place**) and `get_provider_key(provider)` (for `model_gateway.py`'s separate streaming-path lookups, below). Guarded to no-op entirely under `INFERENCE_MODE=local` (Ollama needs none of these keys). Every existing consumer of `INFERENCE_CHAINS` (`model_gateway.py`'s `walk_chain()`, `check_inference_provider_health.py`, `inference_health_handler.py`) needed zero changes — they already read `tier["api_key"]` and now transparently see live values.
4. `backend/app/services/model_gateway.py` — `get_provider_config()` (the separate legacy streaming main-reasoning path, used by `generate_streaming()`) previously imported `GROQ_API_KEY`/`CEREBRAS_API_KEY` as frozen constants; both call sites now call `get_provider_key("groq"/"cerebras")` instead, so this path rotates too. Dead imports removed.
5. `backend/app/main.py` (FastAPI `lifespan`) and `backend/app/workers/arq_worker.py` (`startup`/`shutdown` hooks) each start/cancel their own `asyncio.create_task(start_provider_key_refresh_loop())` — necessary because the two are separate OS processes with independent copies of `config_inference_chains`' module state; a refresh in one never reaches the other. Loop interval: `VAULT_KEY_REFRESH_INTERVAL_SECONDS = 60` (new `config.py` constant, alongside `VAULT_PROVIDER_KEYS_PATH`). Also removed the now-dead `VAULT_POSTGRES_ROLE` constant.
6. **A genuinely pre-existing gap found only because verifying this feature required it**: `arq_worker.py` never called `logging.basicConfig()` (only `main.py` does) — every `logger.info()`/`logger.warning()` anywhere in code running inside the ARQ worker process was silently dropped, with no prior symptom anyone had noticed. Fixed with one `logging.basicConfig(level=...)` call at module load, matching `main.py`'s own level source (`LOG_LEVEL`).

**Verified live, twice — before touching anything and after, via the actual rotation scenario, not a unit-test double:** rebuilt and recreated both `aegis-fastapi` and `aegis-arq` from the real source (`docker compose build` + `up -d`, this project's established pattern for these two bind-mount-less containers) to pick up both the code changes and the corrected `.env` Vault credentials. Confirmed both processes' startup logs show a real AppRole login (`200`) and a real KV v2 read (`200`). Then, **using the Vault root token as an operator would** (the AppRole's own least-privilege policy correctly rejected a write attempt with `403` — confirmed this is working as designed, not a bug, before switching tokens), appended a `-ROTATE-TEST` marker to the live `SAMBANOVA_API_KEY` value directly in Vault. Within one ~60s interval, both `aegis-fastapi` (both of its 2 uvicorn workers) and `aegis-arq` logged `"Provider-key refresh: SAMBANOVA_API_KEY updated from Vault"` — **zero container restarts performed between the rotation and the pickup**. Reverted the marker the same way; both processes logged the same update line a second time, confirming the restore was picked up identically. Final key value confirmed byte-for-byte identical to its pre-test value.

**Test suite:** 19 new tests (`tests/unit/test_vault_client.py` — login/token-caching, `get_secret`/`put_secret` request shape, no-silent-fallback-in-the-client, health check; `tests/unit/test_config_inference_chains.py` — `get_provider_key`, `refresh_provider_keys`'s Vault-success/Vault-failure/partial-secret/local-mode-no-op paths, with a fixture that snapshots and restores the shared `INFERENCE_CHAINS`/`_CURRENT_KEYS` module state so these tests can't leak into the rest of the suite; `tests/unit/test_model_gateway_walk_chain.py` — `get_provider_config()` now sourcing live-patchable keys). Full suite: 347 passed (up from 328), `docker compose config --quiet` valid.

**`OPEN-14` is now genuinely resolved** (not just "direction chosen" — actually built, deployed, and live-verified). Two items from the same guide remain explicitly **not attempted, requiring real human action this environment cannot perform**: Item 4 (generating fresh API keys on 5 external provider dashboards — Groq, Cerebras, SambaNova, Cloudflare, Gemini — requires interactively logging into each with real account credentials) and Item 5 (provisioning a real Oracle Cloud VM — requires a real cloud account and incurs real infrastructure changes). Both are now genuinely easier than before this session: rotation through Vault (Item 4, once new keys exist) needs no container restart at all, exactly the point of building this.

**Affects:** `scripts/setup_vault.py`, `backend/app/infrastructure/vault_client.py`, `backend/app/config_inference_chains.py`, `backend/app/services/model_gateway.py`, `backend/app/main.py`, `backend/app/workers/arq_worker.py`, `backend/app/config.py`, `secrets-share/.env` (Vault AppRole credentials corrected — gitignored, not part of any commit), 3 test files.
...
|---|---|
| `model_gateway.py` | DEC-015, DEC-017, DEC-019, DEC-020, OPEN-01, DEC-058 (`walk_chain()` N-tier engine, bounded streaming fallback), DEC-061 (per-tier `min_max_tokens` floor), DEC-066 (`get_provider_config()` sources live Vault-backed keys via `get_provider_key()`) |
| `vision_task.py` (retrofit — corrected from an earlier "vision_integration.py" error, see DEC-034's history) | DEC-006, DEC-021, DEC-058 (retired inline 2-provider cascade in favor of `walk_chain()`) |
| `config_inference_chains.py`, `providers_cloudflare.py`, `providers_gemini.py`, `inference_health_handler.py`, `check_inference_provider_health.py` (all new) | DEC-058, DEC-061 (`config_inference_chains.py` only — `min_max_tokens` field, judge tier 2 override), DEC-066 (`config_inference_chains.py` only — `refresh_provider_keys()`/`get_provider_key()`, Vault-backed rotation) |
| `circuit_breaker.py` | DEC-064 (`sambanova_main`/`sambanova_judge` override comment corrected from a mislabeled "20 RPM" to the real, already-implemented 20 RPD ceiling) |
| `retrieval_engine.py` (retrofit — `_stage6_crag`) | DEC-015, DEC-019, DEC-020, DEC-057 (`chunk_id`-missing retrievability bug, IMPL_28 Section 5 screenshot surfacing, `is_current` enforcement) |
...
| `chat_handler.py` | DEC-059 (Pub/Sub token relay never existed — found and fixed on the first real end-to-end pass; `answer_text` added to `validation_result`) |
...
| `scripts/aegis_inference_benchmark.py` (rewritten) | DEC-060 (OPEN-05) |
| `vault_client.py`, `scripts/setup_vault.py`, `aegis-vault` (docker-compose) | DEC-046, DEC-051, OPEN-09, DEC-060 (OPEN-14 — repurpose direction chosen), DEC-066 (OPEN-14 resolved — KV v2 built, live-verified, zero-restart rotation) |
| `AEGIS_DATA_CONTRACTS.md`, `IMPL_19_EMPLOYEE_FRONTEND.md`, `frontend/src/types/index.ts` | DEC-065 (OPEN-08 — dead `"correction"` WebSocket message type removed) |
