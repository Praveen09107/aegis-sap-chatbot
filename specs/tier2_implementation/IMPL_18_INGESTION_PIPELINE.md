# IMPL_18: INGESTION PIPELINE
## 11-Stage Document Ingestion: Template Parsing to Vector Storage
## Session 18 Implementation Guide

---

## AGENT INSTRUCTIONS FOR THIS SESSION

You are implementing Session 18: The 11-stage Document Ingestion Pipeline.

Attach: AEGIS_MASTER_REFERENCE.md, AEGIS_DATA_CONTRACTS.md, AEGIS_CONFIGURATION_CONSTANTS.md, AEGIS_DOCUMENT_TEMPLATES.md, and this document.

**Prerequisites:** Sessions 03-17 complete. Qdrant, OpenSearch, PostgreSQL, and BGE service all healthy.

**What this session creates:**
- `backend/app/services/ingestion_pipeline.py` — Complete 11-stage pipeline
- Update `backend/app/handlers/upload_handler.py` — Trigger pipeline after upload
- `tests/unit/test_ingestion_pipeline.py` — Field detection and chunking tests

**The 11 stages:**
```
Stage  1: Magic bytes validation         → confirm file format
Stage  2: Content extraction             → text from DOCX or PDF
Stage  3: Field detection                → parse template field labels
Stage  4: Schema validation              → required fields, format checks
Stage  5: Content validation             → enum values, cross-field checks
Stage  6: Chunking                       → semantic chunks per content_type
Stage  7: Embedding                      → BGE vectors for content + identity
Stage  8: Qdrant ingestion               → upsert to correct collection
Stage  9: OpenSearch indexing            → BM25 index with entity boosting
Stage 10: Knowledge Graph edges          → create document_relationships rows
Stage 11: Documents registry update     → status='active', chunk_count
```

---

## FILE 1: backend/app/services/ingestion_pipeline.py
...
Output: Indexed chunks in Qdrant + OpenSearch + PostgreSQL
...
class IngestionPipeline:
    ...
    async def ingest(self, file_path: str, file_type: str) -> IngestionResult:
        """
        ...
        try:
            ...
            await self._stage11_registry_update(fields, document_id, len(chunks))
            ...
            doc_id = fields.get("DOCUMENT_ID", "unknown") if 'fields' in dir() else "unknown"
    ...
    def _stage4_validate_schema(self, fields: Dict[str, str]) -> List[str]:
        ...
        if content_type == "error_guide":
            ...
            for f in ["CONFIGURATION_NAME", "WHAT_THIS_CONTROLS",
                       "NAVIGATION", "CURRENT_VALUES_AT_SONA_COMSTAR", "CHANGE_PROCESS"]:
                if not fields.get(f):
                    errors.append(f"config requires field: {f}")
            # Reject placeholder values
            current_values = fields.get("CURRENT_VALUES_AT_SONA_COMSTAR", "")
            if "[PLACEHOLDER]" in current_values or not current_values.strip():
                errors.append(
                    "CURRENT_VALUES_AT_SONA_COMSTAR must contain actual Sona Comstar "
                    "configuration values, not placeholders"
                )
    ...
    def _chunk_config(self, fields: Dict, common: Dict) -> List[DocumentChunk]:
        ...
        - config_values chunk: CURRENT_VALUES_AT_SONA_COMSTAR (NEVER SPLIT regardless of length)
        ...
        values_text = (
            f"Configuration: {config_name}\n"
            f"Module: {common['module']}\n"
            f"Current Values at Sona Comstar:\n\n"
            f"{fields.get('CURRENT_VALUES_AT_SONA_COMSTAR', '')}"
        )
        chunks.append(DocumentChunk(
    ...
    async def _stage11_registry_update(
        self, fields: Dict, document_id: str, chunk_count: int
    ) -> None:
        """Update documents_registry with final status and chunk count."""
        transactions = [t.strip() for t in fields.get("TRANSACTIONS", "").split(",") if t.strip()]
                ...
                await conn.execute(
                    ...
                    INSERT INTO documents_registry
                        (document_id, content_type, module, transactions,
                         last_verified_date, verified_by, chunk_count, status)
...
## FILE 2: tests/unit/test_ingestion_pipeline.py
...
class TestFieldDetection:
    ...
    def test_config_fields_detected(self, pipeline):
        ...
        assert "Company Code: 1000" in fields["CURRENT_VALUES_AT_SONA_COMSTAR"]
...
class TestSchemaValidation:
    ...
    def test_config_placeholder_caught(self, pipeline):
        fields = pipeline._stage3_detect_fields(SAMPLE_CONFIG)
        fields["CURRENT_VALUES_AT_SONA_COMSTAR"] = "[PLACEHOLDER] not filled in"
        errors = pipeline._stage4_validate_schema(fields)
...
## UPDATE: upload_handler.py to trigger ingestion

In `backend/app/handlers/upload_handler.py`, update the document upload endpoint:
...
async def upload_document(request: Request, file: UploadFile = File(...)):
    ...
    result = await ingestion_pipeline.ingest(file_path, ext)
...
python -m pytest tests/unit/test_ingestion_pipeline.py -v
```
Expected: All tests pass.

### Test field detection manually
```bash
python3 -c "
from app.services.ingestion_pipeline import IngestionPipeline

pipe = IngestionPipeline()
sample = open('specs/tier1_foundation/AEGIS_DOCUMENT_TEMPLATES.md').read()
# Extract the example section
start = sample.find('DOCUMENT_ID: SD-ERR-001')
end = sample.find('=== END TEMPLATE ===', start)
text = sample[start:end]
fields = pipe._stage3_detect_fields(text)
...
```bash
git add -A
git commit -m "IMPL-18: Ingestion Pipeline - field detection and chunking verified"
```

---
## QUICK ENTRY PARALLEL INGESTION PATH (Added in IMPL_26)
...
2. Replaces Stage 6 (generic chunking) with structure-aware form chunking (IMPL_27).
3. Uses the same underlying services for Stages 5, 7–11:
   entity extraction, embedding, quality scoring, deduplication, Qdrant, OpenSearch.

Entry point: process_form_entry ARQ task (app/tasks/process_form_entry.py)
Triggered by: POST /api/admin/knowledge-entries with publish=true
...
See IMPL_23 for the complete architectural picture.
See IMPL_26 for the ARQ task specification.
See IMPL_27 for the structure-aware chunking engine.
